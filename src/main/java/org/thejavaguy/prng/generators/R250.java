...
public class R250 implements PRNG {
	...
	public R250(long seed) {
		...
		int mask = 0xffffffff;
		for (int j = 0; j < 32; ++j) {
			int indexVektora = 7 * j + 3;
			buffer[indexVektora] &= mask;
			buffer[indexVektora] |= msb;
			mask >>= 1;
		msb >>= 1;
		}
	}
	...
	public int nextInt(int lowerLimit, int upperLimit) {
		if (lowerLimit > upperLimit) {
			throw new IllegalArgumentException(String.format("lowerLimit must be greater than upperLimit, yet values are: lowerLimit = %d, upperLimit = %d", lowerLimit, upperLimit));
		}
